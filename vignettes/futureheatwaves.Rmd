...
- `days.above.abs.threshold.1`: Number of days in the heat wave above a certain absolute threshold (default is $80^oF$, but this and the absolute thresholds used for the following three variables can be customized using the `absolute_threshold` parameter in the `gen_hw_set` function);
- `days.above.abs.threshold.2`: Number of days in the heat wave above a certain absolute threshold (default is $85^oF$);
- `days.above.abs.threshold.3`: Number of days in the heat wave above a certain absolute threshold (default is $90^oF$);
- `days.above.abs.threshold.4`: Number of days in the heat wave above a certain absolute threshold (default is $95^oF$);
...
- `days.above.99.5th`: Number of days in the event above the $99.5^{th}$ percentile year-round value of the variable for the location, using the period specified by the user with the `referenceBoundaries` argument in `gen_hw_set` as a reference for determining these percentiles;
- `first.in.year`: Whether the heat wave was the first to occur in its calendar year in its location (Note: this characteristic is likely not useful in southern hemisphere studies.);
...
```{r}
data(hw_datafr) 
hw_datafr[1:3, c("hw.number", "mean.var", "length", "start.date",
                 "mean.var.quantile", "city")]
```
...
Two parameters in the `gen_hw_set` function can be set to customize the characteristics in the output dataframes. First, the heatwave characteristics output files include the column "mean.seasonal.var", which by default gives the mean value of the temperature in a study location for the months of May--September. If working with a variable or in a location for which different months comprise the most relevant season, you can use the `seasonal_months` argument to customize the months included in this seasonal average, using a vector with numerical values for months (1 = January, 2 = February, etc.). For example, to set the season to the months of June through August in the earlier example of using `gen_hw_set`, you could call:
...
As of version 1.0.3, the package has two other plotting options that are more useful for non-U.S. studies. First, the `map_grid_ggmap` function draws on `ggmap` functions to plot study cities and their matching climate model grid points on map background from Stamen Maps. The function is written to pull a background map just large enough to show all cities and climate model grid points. This mapping function works for study cities anywhere in the world. Here is the code to use to map study cities and matching climate model grid points for the example data included with the package:

```{r message = FALSE, warning = FALSE, fig.width = 5}
map_grid_ggmap(plot_model = "bcc1", out = out)
```

The other new mapping function, `map_grid_leaflet`, allows you to create interactive maps using the `leaflet` package and will also work for study cities anywhere. This function adds pop-ups labeling city locations (using the "city" column in the file designated with the `citycsv` parameter in `gen_hw_set`) and climate model grid point locations. The map, like any leaflet map, allows panning and zooming. 
...
A user could also use this package to explore events that must be lower than some minimum threshold (e.g., cold waves), but it would take some extra coding and conversions, since the functions in this package are written to identify periods above a threshold (for example, the user could multiple all projected temperatures by -1, and then the coldest temperatures would register as being the highest). 

As of version 1.0.3, the package includes the `above_threshold` parameter. By setting this parameter to `FALSE`, you can run the function to identify and characterize extend periods below, rather than above, a certain threshold. For example, you can use this parameter to identify all "cold waves" of two or more days below the 10th percentile temperature in the community. This code also is customized to use thresholds of $20^{o}F$, $15^{o}F$, $10^{o}F$, and $5^{o}F$ for the absolute thresholds used to characterize the number of days in each cold wave below certain absolute thresholds (the `convert_temperature` function from the `weathermetrics` package is used to specify these thresholds in Fahrenheit and convert the to Kelvin before inputting to the `gen_hw_set` function). Further, the `seasonal_months` parameter is used so that the seasonal mean variable calculated for each community is based on the months of December, January, and February. 
...
gen_hw_set(out = "example_results",
           ...
           probThreshold = 0.10,
           absolute_thresholds = convert_temperature(c(20, 15, 10, 5),
                                                     old_metric = "f",
                                                     new_metric = "k"),
           seasonal_months = c(12, 1, 2))
```

When you use the `above_threshold = FALSE` argument, the "heatwave" datasets returned by the function will give columns appropriately converted for periods below a threshold. For example, there are columns in this output for the number of days in each event below the $1^{st}$ and $0.5^{th}$ percentile temperature (or other variable) for the city and for the number of days in the event below some specified absolute thresholds.

Users doing these kinds of extensions will need to pay attention to a couple of points. First, because event periods are usually defined as a string of multiple days exceeding some threshold, the functions in this package may miss the first and last event of the time period. For example, if the first day of the time series is the last day of an event, this function would not identify that event because it lacks data from the earlier days that allow this day to meet the event definition. This issue would lead to, at most, missing two events out of each projection, but should be considered if studying events that might occur near the start and end of projection data. Further, the "first.in.year" column in the output characteristic file is based on calendar year. For some applications (cold waves, heat waves in the southern hemisphere), this characteristic will not identify the first event in the season (i.e., the first cold wave of the cold season).
