...
As of version 1.0.3, the package includes the `above_threshold` parameter. By setting this parameter to `FALSE`, you can run the function to identify and characterize extend periods below, rather than above, a certain threshold. For example, you can use this parameter to identify all "cold waves" of two or more days below the 10th percentile temperature in the community. This code also is customized to use thresholds of 20\textsuperscript{o}F, 15\textsuperscript{o}F, 10\textsuperscript{o}F, and 5\textsuperscript{o}F for the absolute thresholds used to characterize the number of days in each cold wave below certain absolute thresholds. Further, the `seasonal_months` parameter is used so that the seasonal mean variable calculated for each community is based on the months of December, January, and February. 
...
When you use the `above_threshold = FALSE` argument, the "heatwave" datasets returned by the function will give columns appropriately converted for periods below a threshold. For example, there are columns in this output for the number of days in each event below the 1\textsuperscript{st} and 0.5\textsuperscript{th} percentile temperature (or other variable) for the city, the number of days in the event below some specified absolute thresholds, 
