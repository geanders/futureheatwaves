...
city_file_location <- "predictivemodels/cities.csv"
...
map_grid(plot_model = "GFDL", out = out)
map_grid_leaflet(plot_model = "GFDL", out = out)
map_grid_ggmap(plot_model = "GFDL", out = out, lon_transform = TRUE)
...
## Map temperature quantiles
library(viridis)
us_tas <- tas[which(lon > 233 & lon < 294), 
              which(lat > 25 & lat < 50),
              ]

get_locs <- coordinate_output %>%
        mutate(us_lat = lat > 25 & lat < 50,
               us_lon = lon > 233 & lon < 294,
               us_point = us_lat & us_lon)

us_tas <- tas_output[ , which(get_locs$us_point)]
us_mean <- apply(us_tas, 2, mean)

us_coords <- coordinate_output %>%
        filter(lon > 233 & lon < 294 & lat > 25 & lat < 50) %>%
        mutate(lon2 = ifelse(lon > 180, -(360 - lon), lon),
               us_mean = convert_temperature(apply(us_tas, 2, mean),
                                             "k", "f"),
               us_max = convert_temperature(apply(us_tas, 2, max),
                                             "k", "f"),
               us_thresh = convert_temperature(apply(us_tas, 2, 
                                                     quantile, probs = 0.98),
                                             "k", "f"))

ggplot() + 
        borders("state", colour="gray50", fill="gray90") + 
        geom_point(data = us_coords, aes(x = lon2, y = lat,
                                         color = us_mean),
                   size = 2, alpha = 0.8) + 
        scale_color_viridis() + 
        coord_map() + 
