...
local distributions (e.g., 98\textsuperscript{th} percentile temperature
for that location). Here, we provide an overview of working with daily
climate model output data in R. Further, we present the
\pkg{futureheatwaves} package, which we developed to ease the process of
identifying, characterizing, and exploring multi-day extreme events in
climate model output. This package takes as input a directory of
projection files from one or many climate models, identifies all
occurrences of a specific type of extreme event (e.g., heat waves) for
multiple cities using a user-specified event definition, and writes an
output directory of files with these extreme events and their
characteristics (e.g., length, absolute and relative temperatures). The
\pkg{futureheatwaves} package then allows the user to create custom
functions and apply them across all event files; this functionality can
be used either to explore extreme event patterns (e.g., average length
and intensity across different climate models) or to apply
epidemiological estimates to project health impacts under different
scenarios.
}
...
example, with the \code{nc.get.var.subset.by.axes} function in
\pkg{ncdf4.helpers}). Once the user is done reading in data from the
file, the connection can be closed (e.g., with the \code{nc\_close}
function from \pkg{ncdf4}).
...
days since an origin date (\code{as.Date}, \code{as.POSIXct}) do not
return the desired values. The \pkg{PCICt} \citep{PCICt} and
\pkg{ncdf4.helpers} \citep{ncdf4.helpers} packages provide further
...
calendar used, which the \code{nc.get.time.series} function in
\pkg{ncdf4.helpers} use to convert the ``time'' variable in the file to
...
the \code{apply\_all\_models} function allows the user to apply custom
functions across all the extreme event data frames generated by
\code{gen\_hw\_set} to summaries of extreme events across all climate
...
can find the local location of these files using R's \code{system.file}
...
\code{probThreshold} option in \code{gen\_hw\_set}. For example, this
...
definition using the \code{numDays} argument in the \code{gen\_hw\_set}
function (the default is \samp{numDays = 2}, corresponding to two or
...
the definition. The custom function is passed to the \code{gen\_hw\_set}
function using the \code{IDheatwavesFunction} argument. To work
...
are provided in the \pkg{futureheatwaves} package vignette. To increase
...
\CRANpkg{Rcpp} package \citep{Rcpp}. Users should consider a similar
...
The \code{apply\_all\_models} function can also be used to project the
...
have a data frame called \code{city\_mortality} with columns with each
community's identifier (\code{city}) and baseline mortality rate
(\code{base\_mort}), using the call (this example uses hypothetical
baseline mortality rates for the cities and requires the \CRANpkg{dplyr}
package \citep{dplyr}):
...
example of how the \code{apply\_all\_models} function can be used in
conjunction with epidemiologic results to estimate the potential health
impacts of extreme events with climate change.

\section{Customizing dates for projections and reference
...
\code{projectionBoundaries} argument of the \code{gen\_hw\_set}
function.

Further, some of the event characteristics (e.g.,
\code{mean.temp.quantile}, Table \ref{tab:hwcharacteristics}) are
