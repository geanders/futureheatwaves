...
projection files from one or many climate models, identifies all of a
specific type of extreme event (e.g., heat waves) for multiple cities
using a user-specified event definition, and writes an output directory
of files with these extreme events and their characteristics (e.g.,
length, absolute and relative temperatures). The \pkg{futureheatwaves}
package then allows the user to create custom functions and apply them
across all event files; this functionality can be used either to explore
extreme event patterns (e.g., average length and intensity across
different climate models) or to apply epidemiological estimates to
project health impacts under different scenarios.
}
...
The functions and packages just described can be used to work with CMIP5
...
an example of how the \code{apply\_all\_models} function can be used in
